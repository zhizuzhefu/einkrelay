...
许可证为 [Apache-2.0](LICENSE)。完整机器可读接口见 [docs/openapi.yaml](docs/openapi.yaml)，日常调用的可执行示例见 [docs/api-examples.md](docs/api-examples.md)，真实场景示例（把本机 Claude / Codex / Grok / Kimi / GLM / MiniMax / DeepSeek 的额度使用情况做成仪表盘推到 Kindle，[examples/quota-dashboard/](examples/quota-dashboard/) 是可运行的纯 Go 程序）见 [docs/dashboard-example.md](docs/dashboard-example.md)，运行参数见 [docs/configuration.md](docs/configuration.md)，第三方声明见 [docs/third-party.md](docs/third-party.md)。
