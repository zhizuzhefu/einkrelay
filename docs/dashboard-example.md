...
这是「我想把一块仪表盘推到 Kindle 上」的完整实操示例。主示例是一个 **AI 额度仪表盘**：汇总本机 Claude、Codex、Grok、Kimi、GLM、MiniMax、Antigravity、DeepSeek 八家编程服务的额度使用情况推到设备上。它是一个纯 Go 程序，代码在 [examples/quota-dashboard/](../examples/quota-dashboard/)，同时演示了「在 Kindle 上做仪表盘」的推荐做法。
...
go run .                          # image 模式：查询八家额度 → 渲染 PNG → 推送
...
GLM、MiniMax、DeepSeek 走按量 API key，程序从环境变量 `ZHIPUAI_API_KEY`（或 `GLM_API_KEY`）、`MINIMAX_API_KEY`、`DEEPSEEK_API_KEY` 读取；其余五家用本机 CLI 已登录的凭据，不需要额外配置。某一家查不到（未登录、未设 key、网络失败）时面板照常渲染，该服务显示「查询失败」一行，不影响其他家。
...
GLM / MiniMax / DeepSeek 的 API key 从环境变量读，后台启动前确保当前 shell 已加载（如 `source ~/.zshenv`）。停止循环：`kill` 该进程即可；`nohup` 让进程在终端关闭后继续运行，但 Mac 重启后需要重新拉起（要开机自启可以自行包一层 launchd）。
